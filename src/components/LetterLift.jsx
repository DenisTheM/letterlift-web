...
const PKG=[{id:"trial",name:"Trial",letters:1,price:9.9,priceEU:8.9,pl:"9.90",plEU:"8.90",trial:true},{id:"impuls",name:"Impuls",letters:5,price:34.9,priceEU:29.9,pl:"6.98",plEU:"5.98"},{id:"classic",name:"Classic",letters:10,price:59.9,priceEU:49.9,pl:"5.99",plEU:"4.99",pop:true},{id:"journey",name:"Journey",letters:15,price:79.9,priceEU:69.9,pl:"5.33",plEU:"4.66"}];
function getPrice(pk,eu){return eu?pk.priceEU:pk.price;}
function getPPB(pk,eu){return eu?pk.plEU:pk.pl;}
...
export default function App() {
  ...
  const renderStep=()=>{
    ...
    case"context":{const _oc=OCCASION_COPY[d.occasion]||DEFAULT_COPY;
      const CtxSpeech=()=>{const[isRec,setIsRec]=useState(false);const recRef=useRef(null);const toggle=()=>{if(isRec){recRef.current?.stop();setIsRec(false);return;}if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){alert('Spracheingabe wird in diesem Browser nicht unterstützt.');return;}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const r=new SR();r.lang='de-DE';r.continuous=true;r.interimResults=true;recRef.current=r;let final='';r.onresult=(ev)=>{let interim='';for(let i=ev.resultIndex;i<ev.results.length;i++){if(ev.results[i].isFinal)final+=ev.results[i][0].transcript+' ';else interim=ev.results[i][0].transcript;}const base=d.contextText.replace(/🎙️.*$/,'').trimEnd();u('contextText',base+(base?' ':'')+final+(interim?'🎙️'+interim:''));};r.onend=()=>setIsRec(false);r.start();setIsRec(true);};return(<button type="button" onClick={toggle} style={{position:'absolute',right:'10px',bottom:'10px',background:isRec?'#E53E3E':'#EEF4F0',border:'none',borderRadius:'50%',width:'36px',height:'36px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px',transition:'all 0.2s',boxShadow:isRec?'0 0 0 3px rgba(229,62,62,0.3)':'none'}}>{isRec?'⏹':'🎙️'}</button>);};
      return(<div><SH t={_oc.contextQ(rN,isSelf)} s="Je ehrlicher, desto wirkungsvoller."/>
      <div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
        <div><label style={L}>{_oc.contextQ(rN,isSelf)}</label><div style={{position:"relative"}}><textarea style={{...T,paddingRight:"50px"}} value={d.contextText} onChange={e=>u("contextText",e.target.value)} placeholder={_oc.contextPh(rN,isSelf)} onFocus={fc} onBlur={bl}/><CtxSpeech/></div></div>
    ...
    case"memories":{const _oc=OCCASION_COPY[d.occasion]||DEFAULT_COPY;const memQs=_oc.memQ||DEFAULT_COPY.memQ;const memPhs=_oc.memPh||DEFAULT_COPY.memPh;const filledCount=[d.mem1,d.mem2,d.mem3,...(d.memExtra||[])].filter(s=>s&&s.trim().length>=20).length;const totalMems=3+(d.memExtra||[]).length;const pk=d.package||"trial";const minMems=pk==="journey"?4:pk==="classic"?3:pk==="impuls"?2:1;
      const SpeechBtn=({field})=>{const[isRec,setIsRec]=useState(false);const recRef=useRef(null);const toggle=()=>{if(isRec){recRef.current?.stop();setIsRec(false);return;}if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){alert('Spracheingabe wird in diesem Browser nicht unterstützt.');return;}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const r=new SR();r.lang='de-DE';r.continuous=true;r.interimResults=true;recRef.current=r;let final='';r.onresult=(ev)=>{let interim='';for(let i=ev.resultIndex;i<ev.results.length;i++){if(ev.results[i].isFinal)final+=ev.results[i][0].transcript+' ';else interim=ev.results[i][0].transcript;}const prev=typeof field==='number'?(field===0?d.mem1:field===1?d.mem2:d.mem3):(d.memExtra||[])[field-3]||'';const base=prev.replace(/🎙️.*$/,'').trimEnd();const newVal=base+(base?' ':'')+final+(interim?'🎙️'+interim:'');if(typeof field==='number'&&field<3)u(field===0?'mem1':field===1?'mem2':'mem3',newVal);else{const ne=[...(d.memExtra||[])];ne[field-3]=newVal;u('memExtra',ne);}};r.onend=()=>setIsRec(false);r.start();setIsRec(true);};return(<button type="button" onClick={toggle} style={{position:'absolute',right:'10px',bottom:'10px',background:isRec?'#E53E3E':'#EEF4F0',border:'none',borderRadius:'50%',width:'36px',height:'36px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px',transition:'all 0.2s',boxShadow:isRec?'0 0 0 3px rgba(229,62,62,0.3)':'none'}}>{isRec?'⏹':'🎙️'}</button>);};
      ...
      <div style={{padding:"14px 16px",background:"#FFF8F0",borderRadius:"12px",border:"1px solid #F0E4D4",marginBottom:"18px",fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#8B6914",lineHeight:1.6}}>
        <strong>⭐ Hier entstehen die besten Briefe.</strong> {filledCount<minMems?`Mindestens ${minMems} Erinnerung${minMems>1?"en":""} für dein Paket.`:"Nimm dir 5 Minuten."} Jede Erinnerung wird zu einem eigenen, einzigartigen Briefmoment.
        {d.package&&<span> {filledCount>=minMems+1?" 💚 Genug für richtig persönliche Briefe!":filledCount>=minMems?` 🟢 Minimum erreicht! Noch ${minMems>1?"eine":"eine weitere"} Erinnerung macht es noch besser.`:` 🟡 Noch ${minMems-filledCount} Erinnerung${minMems-filledCount>1?"en":""} nötig.`}</span>}
      </div>
      <div style={{padding:"10px 16px",background:"#EEF4F0",borderRadius:"12px",marginBottom:"18px",fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#3D5A4C",display:"flex",alignItems:"center",gap:"8px"}}>🎙️ <strong>Tipp:</strong> Drücke das Mikrofon und erzähl einfach drauflos – oft fällt einem mehr ein als beim Tippen.</div>
      <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
...
function Landing({go,cs}){
  ...
  return(<div style={{minHeight:"100vh",background:"#FBF8F5",fontFamily:"'Lora',Georgia,serif",color:"#2C2C2C",overflowX:"hidden"}}>
    ...
    <section style={{maxWidth:"1000px",margin:"0 auto",padding:"80px 6% 40px"}}><h2 style={{fontSize:"28px",fontWeight:400,textAlign:"center",margin:"0 0 12px"}}>Wähle dein Paket</h2><p style={{fontSize:"15px",fontFamily:"'DM Sans',sans-serif",color:"#8A8480",textAlign:"center",margin:"0 0 40px"}}>Jedes Paket ist eine durchkomponierte Briefserie – kein Brief wie der andere.</p>
      <div style={{background:"#fff",borderRadius:"16px",border:"1.5px dashed #D6CFC8",padding:"24px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"16px",marginBottom:"24px"}}><div><div style={{fontSize:"17px",fontWeight:600,fontFamily:"'DM Sans',sans-serif",color:"#2C2C2C"}}>🔍 Erstmal testen?</div><div style={{fontSize:"14px",fontFamily:"'DM Sans',sans-serif",color:"#6B6360",marginTop:"4px"}}>Ein einzelner Brief – damit du siehst, wie persönlich LetterLift klingt.</div></div><button onClick={()=>go("gift")} style={{background:"transparent",color:"#3D5A4C",border:"2px solid #5B7B6A",borderRadius:"12px",padding:"12px 28px",fontSize:"15px",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>Trial-Brief · {cs}{getPrice(PKG.find(p=>p.id==="trial")||PKG[0],isEU).toFixed(2)}</button></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:"20px"}}>{PKG.filter(p=>!p.trial).map((pk,i)=>{const descs={Impuls:"Kurz und kraftvoll. Perfekt für einen klaren Anlass.",Classic:"Der ideale Bogen. 10 Briefe mit Dramaturgie – unser Bestseller.",Journey:"Für tiefe Begleitung. 15 Briefe über Wochen oder Monate."};return <div key={i} style={{background:"#fff",borderRadius:"18px",padding:"32px 24px",border:pk.pop?"2px solid #5B7B6A":"1.5px solid #E0DAD4",boxShadow:pk.pop?"0 4px 24px rgba(91,123,106,0.12)":"0 2px 12px rgba(0,0,0,0.04)",position:"relative",textAlign:"center"}}>
        {pk.pop&&<div style={{position:"absolute",top:"-12px",left:"50%",transform:"translateX(-50%)",background:"#5B7B6A",color:"#fff",fontSize:"11px",fontFamily:"'DM Sans',sans-serif",fontWeight:600,padding:"4px 16px",borderRadius:"100px",letterSpacing:"0.05em"}}>BELIEBTESTE WAHL</div>}
        <div style={{fontSize:"22px",fontWeight:600,fontFamily:"'DM Sans',sans-serif",color:"#2C2C2C",marginBottom:"4px"}}>{pk.name}</div>
        <div style={{fontSize:"14px",fontFamily:"'DM Sans',sans-serif",color:"#8A8480",marginBottom:"16px"}}>{pk.letters} Briefe</div>
        <div style={{fontSize:"36px",fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"#3D5A4C",marginBottom:"4px"}}>{cs}{getPrice(pk,isEU).toFixed(2)}</div>
        <div style={{fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#8A8480",marginBottom:"16px"}}>{cs}{getPPB(pk,isEU)} pro Brief</div>
        <p style={{fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#6B6360",lineHeight:1.6,margin:"0 0 20px",minHeight:"40px"}}>{descs[pk.name]}</p>
        <button onClick={()=>go("gift")} style={{width:"100%",padding:"14px",background:pk.pop?"linear-gradient(135deg,#3D5A4C,#5B7B6A)":"transparent",color:pk.pop?"#fff":"#3D5A4C",border:pk.pop?"none":"2px solid #5B7B6A",borderRadius:"12px",fontSize:"15px",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer"}}>Jetzt starten</button>
      </div>})}</div>
