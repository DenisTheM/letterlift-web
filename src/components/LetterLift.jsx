  ...
  return(<div style={{minHeight:"100vh",background:"#FBF8F5",fontFamily:"'Lora',Georgia,serif",color:"#2C2C2C",overflowX:"hidden"}}>
    ...
    <section ref={hR} style={{...sa(hV),maxWidth:"1200px",margin:"0 auto",padding:"80px 6% 60px",display:"flex",alignItems:"center",gap:"60px",flexWrap:"wrap"}}>
      <div style={{flex:"1 1 460px",minWidth:"300px"}}><h1 style={{fontSize:"clamp(36px,5vw,56px)",fontWeight:400,lineHeight:1.15,margin:"0 0 20px"}}>Briefe, die<br/>wirklich <span style={{fontStyle:"italic",color:"#5B7B6A"}}>ankommen</span>.</h1><p style={{fontSize:"18px",lineHeight:1.7,color:"#6B6360",margin:"0 0 36px",maxWidth:"480px",fontFamily:"'DM Sans',sans-serif"}}>Für jemanden, der gerade Kraft braucht. Für jemanden, der dir wichtig ist. Oder für dich selbst.</p>
        ...
        <div style={{marginTop:"24px",display:"flex",gap:"8px",flexWrap:"wrap"}}>{["💔 Schwere Zeiten","🎯 Motivation","💪 Selbstvertrauen","🙏 Wertschätzung","🎉 Meilensteine","🌱 Neuanfang"].map(t=><span key={t} style={{fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#5B7B6A",background:"#EEF4F0",padding:"6px 14px",borderRadius:"100px"}}>{t}</span>)}</div></div>
    ...
    <section ref={wR} style={{...sa(wV),maxWidth:"1000px",margin:"0 auto",padding:"80px 6%"}}><h2 style={{fontSize:"28px",fontWeight:400,textAlign:"center",margin:"0 0 40px"}}>So funktioniert's</h2>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"24px"}}>{[{i:"💬",t:"Du erzählst",d:"Was durchlebt diese Person? Was macht sie besonders? Deine Erinnerungen, deine Worte."},{i:"✍️",t:"Wir helfen formulieren",d:"Aus deinem Input entsteht eine Briefserie mit Dramaturgie – unterstützt durch KI, inspiriert durch dich."},{i:"✅",t:"Du entscheidest",d:"Jeden Brief vorab lesen, anpassen oder stoppen. Nichts geht raus ohne dein OK."},{i:"✉️",t:"Post, die berührt",d:"Echtes Papier, echte Briefmarke, echter Briefkasten. Kein Screen, kein Spam."}].map((s,i)=><div key={i} style={{background:"#fff",borderRadius:"16px",padding:"28px 22px",boxShadow:"0 2px 12px rgba(0,0,0,0.04)",textAlign:"center"}}><div style={{fontSize:"32px",marginBottom:"12px"}}>{s.i}</div><div style={{fontSize:"16px",fontWeight:600,fontFamily:"'DM Sans',sans-serif",marginBottom:"8px"}}>{s.t}</div><div style={{fontSize:"13px",fontFamily:"'DM Sans',sans-serif",color:"#7A7470",lineHeight:1.6}}>{s.d}</div></div>)}</div></section>
