...
type DesignVariant = "standard" | "handschrift" | "premium" | "premium_handschrift";

function getDesignVariant(order: any): DesignVariant {
  const isPremium = order.paper_option === "premium" || order.paper_option === "premium_design";
  const isHand = !!order.handschrift_edition;
  if (isPremium && isHand) return "premium_handschrift";
  if (isPremium) return "premium";
  if (isHand) return "handschrift";
  return "standard";
}
...
function buildAddressLines(recipient: any): string {
  const fullName = [recipient.recipient_name, recipient.recipient_last_name].filter(Boolean).join(" ");
...
function renderLetterPage(variant: DesignVariant, letter: any, recipient: any): string {
  ...
  // Fix: Remove greeting from body if it starts with it (prevents double greeting)
  let cleanBody = letter.body;
  if (greeting && cleanBody.trim().startsWith(greeting.replace(/,\s*$/, ""))) {
    cleanBody = cleanBody.trim().substring(cleanBody.trim().indexOf("\n") + 1).trim();
  }
  const body = bodyToHTML(cleanBody);
  ...
  switch (variant) {
    ...
    case "premium_handschrift":
      return `
    <!-- PAGE 2: Premium + Handschrift Letter -->
    <div class="page letter-premium-handschrift ${sizeClass}">
      <div class="frame"></div>
      <div class="corner-tl"></div><div class="corner-tr"></div>
      <div class="corner-bl"></div><div class="corner-br"></div>
      <div class="divider">
...
function getStyles(): string {
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── PREMIUM + HANDSCHRIFT (Indie Flower + Premium frame) ── */
    .letter-premium-handschrift .frame {
      position: absolute; top: 10mm; left: 10mm; right: 10mm; bottom: 18mm;
      border: 0.3pt solid #D4C9B8; pointer-events: none;
    }
    .letter-premium-handschrift .corner-tl,
    .letter-premium-handschrift .corner-tr,
    .letter-premium-handschrift .corner-bl,
    .letter-premium-handschrift .corner-br {
      position: absolute; width: 8mm; height: 8mm; pointer-events: none;
    }
    .letter-premium-handschrift .corner-tl { top: 10mm; left: 10mm; border-top: 0.6pt solid #C4B494; border-left: 0.6pt solid #C4B494; }
    .letter-premium-handschrift .corner-tr { top: 10mm; right: 10mm; border-top: 0.6pt solid #C4B494; border-right: 0.6pt solid #C4B494; }
    .letter-premium-handschrift .corner-bl { bottom: 18mm; left: 10mm; border-bottom: 0.6pt solid #C4B494; border-left: 0.6pt solid #C4B494; }
    .letter-premium-handschrift .corner-br { bottom: 18mm; right: 10mm; border-bottom: 0.6pt solid #C4B494; border-right: 0.6pt solid #C4B494; }
    .letter-premium-handschrift .divider {
      position: absolute; top: 20mm; left: 50%; transform: translateX(-50%);
      display: flex; align-items: center; gap: 3mm;
    }
    .letter-premium-handschrift .divider .diamond {
      width: 2mm; height: 2mm; background: #C4B494; transform: rotate(45deg);
    }
    .letter-premium-handschrift .divider .rule {
      width: 18mm; height: 0.3pt; background: #C4B494;
    }
    .letter-premium-handschrift .content { padding: 30mm 28mm 0; }
    .letter-premium-handschrift .date,
    .letter-premium-handschrift .greeting,
    .letter-premium-handschrift .body,
    .letter-premium-handschrift .signoff {
      font-family: 'Indie Flower', cursive; font-size: 12.5pt; color: #3A3428;
      line-height: 8.5mm;
    }
    .letter-premium-handschrift .date { color: #B0A590; text-align: right; margin-bottom: 12mm; }
    .letter-premium-handschrift .greeting { margin-bottom: 8.5mm; font-style: italic; }
    .letter-premium-handschrift .body p { margin-bottom: 8.5mm; }
    .letter-premium-handschrift .body p:last-child { margin-bottom: 0; }
    .letter-premium-handschrift .signoff { font-style: italic; margin-top: 8.5mm; }
    .letter-premium-handschrift .footer-area {
      position: absolute; bottom: 22mm; left: 20mm; right: 20mm;
      display: flex; justify-content: center; align-items: center; gap: 4mm;
    }
    .letter-premium-handschrift .footer-area .fl {
      width: 14mm; height: 0.3pt; background: #D4C9B8;
    }
    .letter-premium-handschrift .footer-area .ll {
      font-family: 'DM Sans', Arial, sans-serif; font-size: 6pt; font-weight: 500;
      text-transform: uppercase; letter-spacing: 3px; color: #C4B8A6;
    }
    ...
    /* Size modifiers: premium_handschrift */
    .size-short.letter-premium-handschrift .date       { font-size: 14.5pt; }
    .size-short.letter-premium-handschrift .greeting   { font-size: 14.5pt; }
    .size-short.letter-premium-handschrift .body       { font-size: 14.5pt; }
    .size-short.letter-premium-handschrift .body p     { margin-bottom: 7mm; }
    .size-short.letter-premium-handschrift .signoff    { font-size: 14.5pt; }

    .size-long.letter-premium-handschrift .date        { font-size: 11pt; }
    .size-long.letter-premium-handschrift .greeting    { font-size: 11pt; line-height: 1.75; }
    .size-long.letter-premium-handschrift .body        { font-size: 11pt; line-height: 1.75; }
    .size-long.letter-premium-handschrift .body p      { margin-bottom: 3.5mm; }
    .size-long.letter-premium-handschrift .signoff     { font-size: 11pt; line-height: 1.75; }`;
}
...
function generateLetterHTML(letter: any, recipient: any, order: any): string {
  ...
  const fontMap: Record<DesignVariant, string> = {
    ...
    premium_handschrift: "family=Indie+Flower",
  };
  ...
  try {
    ...
    try {
      if (order.buyer_email) {
        ...
        const { subject, html: eHtml } = letterSentEmail(order, letterIndex, name);
        await sendEmail(order.buyer_email, subject, eHtml);
      }
    } catch (e) { console.error("[send-letter] Email failed:", e); }
